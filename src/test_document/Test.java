...
//					}
	...
	public static void a(String... args)
	{
		try
		{
			...
			{"firstName":"John", "familyName":"Doe", "birthDate": LocalDate(1982-08-15), "gender":"male", "4":true, "nulls": [null,null,null,null,null,"female","male",null,null,null,null,null], "zeros": [0,0,0,0,0,255,255,255,0,0,0,0,0]}
            """);
			...
			byte[] dicData = dic.toByteArray(doc);

			hexDump(dicData);

			Document doc2 = dic.fromByteArray(dicData);
			System.out.println(doc);
			System.out.println(doc2);

			hexDump(doc.toByteArray());

//			Document doc3 = new Document().fromByteArray(doc.toByteArray());
...
//			System.out.println(doc.toJson(false));
//			Document doc = new Document().fromJson(new String(new FileInputStream("D:\\terifan\\BundleEditor\\src\\org\\terifan\\raccoon\\document_viewer\\test\\test20.json").readAllBytes()));
...
//			System.out.println("-".repeat(200));
			...
			dic.add("Male");
			dic.add("Female");
			dic.add(LocalDateTime.parse("2014-05-15T22:58:28"));

//			Marshaller.filter(k->!k.equals("password")).shorten(dic).signature("".getBytes()).compress(Marshaller.BEST).compact(true).marshall(doc).to(baos).asBinary();
			hexDump(dic.toByteArray(doc));

			System.out.println(dic.fromByteArray(dic.toByteArray(doc)));

			System.out.println(doc.toByteArray().length);
			System.out.println(dic.toByteArray(doc).length);
		}
		catch (Throwable e)
		{
			e.printStackTrace(System.out);
