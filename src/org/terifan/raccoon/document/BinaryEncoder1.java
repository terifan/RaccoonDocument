...
class BinaryEncoder1
{
	...
	public BinaryEncoder1(OutputStream aOutputStream)
	{
...
//		System.out.println(structs);
	...
	void writeDocument(Document aDocument) throws IOException
	{
		...
		for (Entry<String, Object> entry : aDocument.entrySet())
		{
			...
			format.append(type.ordinal() + "," + key.length() + "," + key);
		}
		...
		writeHeader(3 + mObjectPosition.size(), buf.length);
		mOutputStream.writeBytes(buf);
...
//		try (DeflaterOutputStream dos = new DeflaterOutputStream(baos, deflater))
	...
	private void writeHeader(int aOffset, int aIndex) throws IOException
	{
//		mOutputStream.writeUnsignedVarint(aIndex);
//		mOutputStream.writeUnsignedVarint(aOffset);
